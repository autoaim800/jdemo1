...
public class SquareTableTest extends TestCase {
	...
	public void testGetDirs4() {
		...
		assertEquals("dir length does not match", RobotHelper.DIR4.length,
				tableDirs.length);

		for (int i = 0; i < tableDirs.length; i++) {
			assertEquals("dir value does not match", RobotHelper.DIR4[i],
					tableDirs[i]);
		}
	}
	...
	public void testGetDirs6() {
		...
		assertEquals("dir length does not match", RobotHelper.DIR6.length,
				tableDirs.length);

		for (int i = 0; i < tableDirs.length; i++) {
			assertEquals("dir value does not match", RobotHelper.DIR6[i],
					tableDirs[i]);
		}
	}
